...
public class DBHelper {
    ...
    private static final String get_querry = "SELECT " + WordListDB.WordListSchema.FeedEntry.COLUMN_NAME_TITLE + " FROM " + WordListDB.WordListSchema.FeedEntry.TABLE_NAME +
    ...
    public static void writeData(Context context, int lowerBound, int upperBound) {
        ...
        if (!sharedPref.getBoolean(key, false)) {
            ...
            try {
                ...
                String str;
                while ((str = buf.readLine()) != null && str.length() >= lowerBound && str.length() <= upperBound) {
                    ...
                    if (cur.getCount() == 0) { // if word does not exist then add
                        ...
                        if (sqLiteDatabase.insert(WordListDB.WordListSchema.FeedEntry.TABLE_NAME,
                                null, values) == -1) {
